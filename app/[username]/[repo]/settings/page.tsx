...
export default function RepoSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/repository/${encodeURIComponent(repoFullName)}/settings`)
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(json.error || "Failed to load repo settings");
  ...
  async function handleSave() {
    ...
    setSuccessMsg(null);
    try {
      const res = await fetch(`/api/repository/${encodeURIComponent(repoFullName)}/settings`, {
        method: "PATCH",
  ...
  function handleChange(key: keyof RepoSettings, value: any) {
    setSettings((s) => (s ? { ...s, [key]: value } : s));
    setSuccessMsg(null);
  }

  if (loading && !settings) return <div>Loading...</div>;
  if (errMsg && !settings) return <div className="text-red-600">Error: {errMsg}</div>;
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      ...
      >
        <div className="mb-4">
          <label className="block mb-2 font-medium">Environments (comma separated)</label>
          <Input
        ...
        <div className="mb-4">
          <label className="block mb-2 font-medium">Setup Commands (one per line)</label>
          <Textarea
