...
type SetupSummary = { steps: SetupStep[]; manualSteps: string[]; warnings?: string[] }
...
async function handler(baseDir: string): Promise<SetupSummary> {
  ...
  if ([hasPnpm, hasYarn, hasNpm].filter(Boolean).length > 1) {
    warnings.push("More than one JS package manager detected. Using first found.")
  }
  ...
  if (fileExists(baseDir, "docker", "docker-compose.yml")) {
    steps.push({ command: "docker compose -f docker/docker-compose.yml up -d", required: false })
  ...
  return { steps, manualSteps, warnings: warnings.length > 0 ? warnings : undefined }
}
