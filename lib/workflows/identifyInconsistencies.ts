...
import { getPullRequest, getPullRequestDiff, getPullRequestComments, getPullRequestReviews } from "@/lib/github/pullRequests";
 ...
 */
export async function identifyInconsistencies({ repoFullName, pullNumber, openAIApiKey }: { repoFullName: string; pullNumber: number; openAIApiKey?: string }) {
  // 1. Fetch PR data
  ...
  if (!issueNumber) {
    console.warn("Could not determine the issue number for this PR. Inconsistency analysis will be limited.");
  }

  // 2. Fetch Issue and Plan context (optional if not found)
  let issue = null, plan = null;
  if (issueNumber) {
    try {
