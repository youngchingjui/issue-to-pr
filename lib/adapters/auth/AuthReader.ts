...
export const nextAuthReader: AuthReaderPort = {
  async getAuthenticatedUser(): Promise<AuthenticatedUser | null> {
    const session = await auth()
    if (!session?.token?.profile?.login || !session?.token?.sub) return null
    return {
      id: session.token.sub,
      githubLogin: session.token.profile.login,
  ...
  async getAuth() {
    const session = await auth()
    if (!session?.token?.profile?.login || !session?.token?.sub) {
